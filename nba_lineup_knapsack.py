...
arg_p.add_argument('-csv', default='DKSalaries.csv')
...
def lineup_knapsack(players):
    ...
    for player in players:
        ...
        if counter['PF'] < limits['PF'] and current_lineup_cap + sal <= cap and pos in ['PF','PF/C']:
            ...
            current_lineup_cap += sal
            continue
        if counter['C'] < limits['C'] and current_lineup_cap + sal <= cap and pos in ['C']:
            lineup.append(player)
            counter['C'] += 1
            current_lineup_cap += sal
            continue
        if counter['G'] < limits['G'] and current_lineup_cap + sal <= cap and pos in ['PG','SG']:
            ...
            current_lineup_cap += sal 
            continue
        '''if counter['F'] < limits['F'] and current_lineup_cap + sal <= cap and pos in ['SF','PF','SF/PF']:
            lineup.append(player)
            counter['F'] = counter['F'] + 1
            current_lineup_cap += sal 
            continue
        '''
        if counter['UTIL'] < limits['UTIL'] and current_lineup_cap + sal <= cap and pos in ['PG','SG','SF', 'PF']:
            lineup.append(player)
